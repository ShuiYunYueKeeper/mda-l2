...
│   │   │   ├── workspace-prefs.js # 工作区 / 会话 / 窗口几何偏好
...
│   ├── prompts/               # AI 协作记录（含 prompt-18 设置/清空批注迭代）
...
- **导出 HTML / PDF / Word**：GUI 菜单「文件 → 导出 HTML / PDF / Word」（复用公众号复制的内联样式与图片内嵌逻辑；Word 经 `html-to-docx`）
- **设置 / 自动保存**：菜单「视图 → 设置…」（`Ctrl+,`）配置自动保存（关闭 / 失焦 / 每 30 秒 / 每 60 秒，仅已落盘文件；未命名不自动另存）、记住上次会话、记住界面习惯（含窗口大小）
...
- **图片 / 流程图缩放**：点击预览中的图片或流程图进入缩放遮罩，支持滚轮缩放（0.3×–8×）、拖拽平移（边界受限不丢失）、双击复位、`Esc`/× 关闭；工具栏「复制」或 `Ctrl+C` — **图片**写入剪贴板图片，**流程图**复制 Mermaid 源码。
...
- **批注管理**：增 / 删 / 改 / 筛选（按状态、级别、标签），写操作复用 core writer（原子写入 + 源文件保护）；支持**选区级批注**（预览/源码拖选 + 右键，含代码块/表格内映射）；批注栏状态筛选**默认仅 open**；支持**一键清空当前文件全部批注**（确认后执行，dirty 时禁用）。
...
- **导出 HTML / PDF / Word**：菜单「文件 → 导出 HTML / PDF / Word」，单文件离线分发。
- **设置**：菜单「视图 → 设置…」（`Ctrl+,`）— **自动保存**（关闭 / 失焦 / 每 30 秒 / 每 60 秒，默认关闭，仅已落盘文件）、**记住上次会话**（文件列表 + 当前文件；关闭则下次进欢迎页）、**记住界面习惯**（窗口大小/位置 + 批注栏/编辑栏/大纲/侧栏展开态与分栏宽度）。打开设置时菜单不可点开。
- **自动更新**：打包版「帮助 → 检查更新」。
- **界面语言**：菜单「视图 → 界面语言」— 跟随系统 / 中文 / English；偏好写入用户目录，切换后立即刷新菜单与 UI。
- **工作区文件侧栏**：`Ctrl+Alt+O` 打开文件夹后展示 Markdown 树；侧栏可拖宽、双击右缘复位；标题栏 **✕** 清空文件列表（关闭工作区侧栏，**不删除**磁盘文件）；悬停显示**全路径**；↑↓ 切换文档；右键与快捷键支持复制文件名、复制/剪切/粘贴、重命名（`Ctrl+R`，仅改主文件名）、删除（`Delete`）、拖动移动（`Ctrl` 拖动为复制）；重名时弹窗选择覆盖/自动重命名/取消；`Ctrl+Z` 撤销最近一次文件操作；侧栏宽度、收起状态、目录展开等受「记住界面习惯」控制；上次工作区受「记住上次会话」控制。
- **文档大纲**：位于预览区**左侧**，可收起/展开（收起后左侧窄栏按钮）、可拖宽；hover 大纲区才显示与正文的分隔线；点击跳转；滚动预览或点击编辑/预览时同步高亮当前标题；正文栏宽恒定，收放大纲不重排跳动。
- **最近打开与启动**：菜单「最近文件」与欢迎页列表悬停/展示可辨全路径；可清空最近列表（不清当前文档）；「记住上次会话」关闭或最近列表为空时下次启动进入**起始页**，且不会因恢复工作区而自动打开树内第一个文件。
- **编辑区偏好**：用户手动收起编辑栏后，切换文档保持收起（受「记住界面习惯」控制）。
