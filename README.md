...
- **自动保存**：菜单「文件 → 自动保存」可选关闭 / 失焦 / 每 30 秒 / 每 60 秒（仅已落盘文件；未命名不自动另存）
...
- **自动保存**：菜单「文件 → 自动保存」— 关闭 / 失焦保存 / 每 30 秒 / 每 60 秒（默认关闭；仅 `open` 且已有路径时生效）。
- **自动更新**：打包版「帮助 → 检查更新」。
- **界面语言**：菜单「视图 → 界面语言」— 跟随系统 / 中文 / English；偏好写入用户目录，切换后立即刷新菜单与 UI。
- **工作区文件侧栏**：`Ctrl+Alt+O` 打开文件夹后展示 Markdown 树；侧栏可拖宽、双击右缘复位；标题栏 **✕** 清空文件列表（关闭工作区侧栏，**不删除**磁盘文件）；悬停显示**全路径**；↑↓ 切换文档；右键与快捷键支持复制文件名、复制/剪切/粘贴、重命名（`Ctrl+R`，仅改主文件名）、删除（`Delete`）、拖动移动（`Ctrl` 拖动为复制）；重名时弹窗选择覆盖/自动重命名/取消；`Ctrl+Z` 撤销最近一次文件操作；记住侧栏宽度、收起状态、上次工作区与目录展开状态。
- **文档大纲**：位于预览区**左侧**，可收起/展开（收起后左侧窄栏按钮）、可拖宽；hover 大纲区才显示与正文的分隔线；点击跳转；滚动预览或点击编辑/预览时同步高亮当前标题；正文栏宽恒定，收放大纲不重排跳动。
- **最近打开与启动**：菜单「最近文件」与欢迎页列表悬停/展示可辨全路径；可清空最近列表（不清当前文档）；列表为空时下次启动进入**起始页**，且不会因恢复工作区而自动打开树内第一个文件。
...
其余素材（侧栏操作/冲突弹窗/语言切换/大纲收起等）暂不补充，清单见 [`docs/screenshots/README.md`](docs/screenshots/README.md)。
