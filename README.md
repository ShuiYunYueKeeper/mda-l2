...
- **三栏布局**：工具栏「编辑」「批注」两个独立开关，中间预览常驻；两者可同时展开为 **源码 ｜ 预览 ｜ 批注** 三栏平铺。
- **源码编辑**：编辑栏内直接修改 Markdown 源码，右侧预览实时更新（防抖）；`Ctrl+S` 整篇原子写回并保留原换行风格。保存前若检出「疑似批注但格式不正确」的行会弹窗提示；存在未保存修改时禁用批注增删改，避免与磁盘写入冲突。
